    ...
    !Darsh 2017: Adding power at given k
    if((P%kprime)*(1.d0-0.1)  < k .and. k < (1.d0+0.1)*(P%kprime) ) then  
        ScalarPower=P%ScalarPowerAmp(ix)*(1.d0 + P%epsAmp)*exp(lnrat*( P%an(ix)-1 + lnrat*(P%n_run(ix)/2 + P%n_runrun(ix)/6*lnrat)))
            print *, 'I have added the power at', k
    else  
    ...
    !Darsh 2017 
    InitPower%epsAmp = Ini_Read_Double_File(Ini,'epsX',InitPower%epsAmp)
    InitPower%kprime = Ini_Read_Double_File(Ini,'kXpivot',InitPower%kprime)
